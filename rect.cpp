...
bool Rect::isOverlapping(const Rect &r) const {
    ...
    if (r.getLeftX() > this->getRightX() || this->getLeftX() > r.getRightX())
        return false;
    if (r.getTopY() > this->getBottomY() || this->getTopY() > r.getBottomY())
        return false;
    return true;
}
...
void Rect::draw() const {
    ...
    glRectf(this->getLeftX(),this->getBottomY(), this->getRightX(), this->getTopY());
}